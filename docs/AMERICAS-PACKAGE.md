...
| **The `wari_culture` → "Inca" render** (11 monarchies) | `peruvian.txt:693`, `country_ranks.txt:1917` | A culture/language question, not a 1066 one. **POP-PHASE** (§H) |
| **CLM's `teco_culture` on zero locations** | `mesoamerica.txt:383`, CLM landed with 7 | **POP-PHASE** — the theater's only registry/ground mismatch on a landed tag |
...
| **The `wari_culture` → `quechuan_language` → "Inca" render** on 11 monarchies | `peruvian.txt:693`; `country_ranks.txt:1917` | **POP-PHASE.** A culture-mapping question that is equally wrong at 1337 |
| **CLM's `culture_definition = teco_culture` on ZERO of 20,922 locations** | `mesoamerica.txt:383`; CLM landed with 7 | **POP-PHASE.** The theater's only landed-tag registry/ground mismatch — one, against Perm/Vyatka's eleven |
